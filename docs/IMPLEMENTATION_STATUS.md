...
- 无主窗口状态反馈优先使用 Windows App SDK 系统通知；通知静音、使用系统默认短时展示、30 秒后过期，并以固定 Tag/Group 替换上一条运行提示。管理员模式或运行环境不支持 App SDK 通知时回退到现有托盘图标气泡；用户或组策略明确禁用通知时不绕过系统设置。设置保存仍使用绑定设置窗体的 AntdUI 轻提示。
